...
from experiments.models import HistoricoQuestoes
...
def _invoke_xblock_handler(request, course_id, usage_id, handler, suffix, user):
    ...
    try:
        ...
        dtReq = request.raw_post_data

        print "dtReq: ", dtReq

        dtReqD = QueryDict(request.body, encoding='utf8')

        data = QueryDict(request.raw_post_data)


        ManipString = dtReq.split('&')
        ...
        for FIELD in ManipString:
            HIST = FIELD.split('=')[1]
            HISTString += HIST+', '
            HistList.append(HIST)

        print "HISTString: ", HISTString
        print "HistList: ", HistList
        print "HistString: ", HIST
        print "Somente uma opção: ", dtReqD[dtReqD.keys()[0]]



        if len(HistList) == 1:
            campoHist= dtReqD[dtReqD.keys()[0]]
            print "OP 1: ", campoHist
        else:
            campoHist = HistList
            print "OP2: ", campoHist


        print "CampoHist: ", campoHist


        # for histL in HistList:
        #     print "HistChoice: ", histL


        # itens = dict(item.split('=', 1) for item in dtReq.split('&'))

#         print "Itens: ", itens
# #        print "DATA: ", request.POST['data']
#         print "Keys: ", data.keys()
#
#         print "Data keys; ", data[data.keys()[0]]
#         print "Data keys; ", data[data.keys()[1]]




        # print "Request Body: ", QueryDict(request.body)
        #
        # print "DATA: ", data
        # print "KEY: ", QueryDict(request.body).keys()[0].replace('input_','').replace('[]','')
        # print "KEY: ", QueryDict(request.body).keys()
        # print "KEY: ", QueryDict(request.body).keys()[0]
        ...
        campo = QueryDict(request.body).keys()[0].replace('input_','').replace('[]','')
        # vals = dtReqD[dtReqD.keys()[0]]

        # val=''

        # try:
        #     print "Vals: ", vals, "  ",  dtReqD[dtReqD.keys()[1]]
        #
        #     if len(vals) > 1:
        #         for v in vals:
        #             val += ',' +v
        ...
        #
        print "Campo: ", campo
        print "Val digitado: ", campoHist # Isto ja esta funcionando, falta testar com as outras variaveis

        hist = HistoricoQuestoes()
        hist.campo = campo
        # hist.valor = varsR[1]
        hist.valor = unicode(campoHist)
        ...
        print "Deu erro ao gravar o historico"
