...
def peer_grading(request, course_id):
    ...
    found_module, problem_url = find_peer_grading_module(course, request.user)
    if not found_module:
        error_message = _("""
...
def combined_notifications(request, course_id):
    ...
    course_key = SlashSeparatedCourseKey.from_deprecated_string(course_id)
    course = get_course_with_access(request.user, 'load', course_key)
    notifications = open_ended_notifications.combined_notifications(course, request.user)
